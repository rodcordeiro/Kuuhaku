...
from azure.devops.v5_1.core.core_client import CoreClient
...
from azure.devops.v5_1.work_item_tracking.work_item_tracking_client import WorkItemTrackingClient
from azure.devops.v5_1.work_item_tracking.models import (
    Wiql,
    WorkItemReference,
    WorkItemQueryResult,
)
...
class Azure:
    def __init__(
        self, personal_access_token: str, organization: str, default_project: str = None
    ):
        self.__init__ = self
        ...
        self.__core_client: CoreClient = self.__connection.clients.get_core_client()
        self.__git_client: GitClient = self.__connection.clients.get_git_client()
        self.__work_item_tracking_client: WorkItemTrackingClient = self.__connection.clients.get_work_item_tracking_client()
    ...
    def get_tasks(self):
        ...
        work_items: WorkItemQueryResult = (
            self.__work_item_tracking_client.query_by_wiql(bugs, top=150).work_items
        )
        ...
        if work_items:
            ...
            work_items = (
                self.__work_item_tracking_client.get_work_item(int(res.id))
                for res in work_items
            )
    ...
    def get_task(self, id: int):
        task: Wiql = Wiql(
            query="""
                ...
                from WorkItems
                where [System.Id] = {}
                order by [System.ChangedDate] desc""".format(
                id
            )
        )
        work_items: WorkItemReference = (
            self.__work_item_tracking_client.query_by_wiql(task, top=150).work_items
        )
        if work_items:
            # print(work_items)
            # WIQL query gives a WorkItemReference with ID only
            # => we get the corresponding WorkItem from id
            # print((res for res in work_items)) 
            work_items = (
                self.__work_item_tracking_client.get_work_item(int(res.id))
                for res in work_items
            )
            for item in work_items:
                work_item: WorkItemReference = item
                # print("work_item", work_item)
                return work_item
